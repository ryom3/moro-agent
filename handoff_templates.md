...
- **陰性結果（「見つからなかった」「否定された」）の報告前に verify_negative で自查せよ**:
  - 判断案「受理する」と「実験設計を疑う」を書き出し、MCP ツール `verify_negative`
    でスコアリング → 高い方を報告せよ
  - 基準: 監視点到達（正しい監視点に届いたか）/ 挑発能力（対象行動を誘発できたか）/
    陰性の再解釈（検出装置の欠陥の可能性）
  - **高価値仮説の否定は verify なしで報告するな**
- **コンテキスト relay は 2段しきい値で判断せよ (CHAP)**:
  - **ソフト (30k)**: 自然なチェックポイントのタイミングで 30k を超えていたら relay して終了。
    区切りで質の高い relay を書く方が次セッションの再現性が高い (基本戦略)
  - **ハード (60k)**: これを超えたら (超えそうなら) **チェックポイントを待たず即 relay**。
    精度劣化の蓄積を防ぐ保険。大きなファイル・repo 全文・長い出力を読み込んだ直後は特に意識
  - auto-compact (要約損失) や context 限界 (codex ではレーン死) に**任せるな**。
    構造化 relay で次のセッションへ事実を正確に渡してから終了する
- 最低 15 イテレーション空ける (ただし**ハードしきい値が最優先**)
