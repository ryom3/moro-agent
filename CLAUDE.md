...
### relay のタイミング (コンテキストしきい値が最優先)
- **コンテキスト使用量が RELAY_CONTEXT_TOKENS (config, 既定 30k) を超えたら (超えそうなら)、
  自然なチェックポイントを待たずに即座に relay して終了せよ** (CHAP の中核)
...
- 最低 15 イテレーション間隔を空けること (ただし**コンテキストしきい値が最優先**)
